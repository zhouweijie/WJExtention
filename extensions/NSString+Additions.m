...
- (NSString *)localCachePath
{
    ...
    NSString * cachePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:@"ZXContentsCache"];
    [JiemianPathHelper createPathIfNecessary:cachePath];
    return [NSString stringWithFormat:@"%@/%@.%@",cachePath,filename,self.pathExtension];
}
...
@implementation NSString (ResetSort)

- (NSDictionary *)URLConvertToDictionary
{
    return [self convertToDictionaryUrlComponentsSeparatedByString:@"?"];
}
/**
 *  传入要分隔开Http的url字符串
 *
...
- (NSDictionary *)convertToDictionaryUrlComponentsSeparatedByString:(NSString *)separator
{
    ...
    NSRange rangeParam = [self rangeOfString:separator];
    if (rangeParam.location != NSNotFound && rangeParam.location != 0) {
        
        NSString *paramsStr = [self substringWithRange:NSMakeRange(rangeParam.location+rangeParam.length, self.length-rangeParam.location-rangeParam.length)];
        mDic = [paramsStr convertToDictionaryComponentsSeparatedByString:@"&"];
...
/**
 *  access_token=DF03AF1D44B1F525A7CEC0D5088A50BB&expires_in=7776000
 *
 *  格式化后：@{access_token:DF03AF1D44B1F525A7CEC0D5088A50BB,
 *            expires_in:7776000}
 *
...
- (NSDictionary *)convertToDictionaryComponentsSeparatedByString:(NSString *)separator
{
    ...
    NSString *filterStr = [self stringByReplacingOccurrencesOfString:@"'" withString:@""];
    NSArray *paramsArr = [filterStr componentsSeparatedByString:separator];
    
    for (NSString *obj in paramsArr) {
        NSRange range = [obj rangeOfString:@"="];
        if (range.location != NSNotFound && range.location != 0) {
            NSRange keyRange = NSMakeRange(0, range.location);
            NSRange valueRange = NSMakeRange(range.location+range.length, obj.length-range.location-range.length);
            [mDic setObject:[obj substringWithRange:valueRange] forKey:[obj substringWithRange:keyRange]];
        }
    }
...
- (NSString *)customRangeOfString:(NSString *)string separated:(NSString *)separatedByString
{
    ...
    NSRange range = [string rangeOfString:separatedByString];
    if (range.location != NSNotFound && range.location != 0) {
        NSRange valueRange = NSMakeRange(range.location+range.length, string.length-range.location-range.length);
        returnString = [string substringWithRange:valueRange];
    }
    return returnString;
}
...
@implementation NSString (RegexCategories)
...
- (BOOL)isMatchForType:(RegexType)type;
{
    ...
    switch (type) {
        case RegexTypePhone: // 手机号码
        {
//            matchStr = @"^((13[0-9])|(15[^4,\\D])|(18[0,0-9]))\\d{8}$";
//            matchStr = @"^1([0-9])\\d{9}$";
            matchStr = @"^[0-9]{11}$";
        }
            break;
        case RegexTypePassword: // 密码长度（6-16）位
        {
//                        matchStr = @"^[a-zA-Z0-9]{6,16}$";
                        matchStr = @"^[A-Za-z0-9@!#$%^&*.~]{6,16}$";
//            matchStr = @"^[\\s\\S]{6,16}$";
        }
            break;
        case RegexTypeEMail: // 邮箱
        {
            matchStr = @"\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*";
        }
            break;
        case RegexTypeNickName: // 昵称  2~12个中英文或数字字符
        {
            matchStr = @"^[a-zA-Z0-9\u4e00-\u9fa5]{2,12}$";
        }
            break;
        case RegexTypeCutImageService:
        {
            matchStr = @"^http://([a-zA-Z0-9]+).f.itc.cn";
        }
            break;
        case RegexTypeContent:
        {
            matchStr = @"^[\\s]+$";
        }
            break;
    ...
    return [self isMatch:RX(matchStr)];
}
