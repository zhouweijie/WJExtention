...
//  news
//
//  Created by zhouweijie on 2019/8/1.
...
extension String {
    //MARK: -传进去字符串,生成二维码图片
    func QRCodeImage(logo: UIImage?, logoSize: CGSize?) -> UIImage {
        //创建滤镜
        let filter = CIFilter(name: "CIQRCodeGenerator")
        filter?.setDefaults()
        //将url加入二维码
        filter?.setValue(self.data(using: .utf8), forKey: "inputMessage")
        //取出生成的二维码（不清晰）
        if let outputImage = filter?.outputImage {
            ...
            if let image = logo {
                ...
                //合成图片
                let newImage = syntheticImage(qrCodeImage, iconImage: image, width: logoSize?.width ?? 50, height: logoSize?.height ?? 50)
